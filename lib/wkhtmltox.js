...
function wkhtmltox(opts) {
    ...
    // Apply sanity checks, override requested values if requested value is too crazy.
    ...
    // HTML to PDF Workers
    // A pool of workers is maintained with a periodic monitor that adds workers when needed.
    var workers = [];

    // Periodically check the number of workers in workers[], if it's below opts.maxWorkers, add more.
    ...
    function launchWorker() {
        var child = spawn("/bin/sh", [ "-c", [ self.wkhtmltopdf, "--quiet", "--read-args-from-stdin" ].join(" ") + "| cat" ], { stdio: ['pipe', 'pipe', process.stderr] });
    ...
    self.image = function image(input, options, callback) {
        ...
        var isUrl = /^(https?|file):\/\//.test(input);
        var child = spawn("/bin/sh", [ "-c", args.join(" ") + " | cat" ], { stdio: ['pipe', 'pipe', process.stderr] });
